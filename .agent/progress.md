...
- 箇条書きは 1 列の長い黒丸にしない。資格は `<table>`、スキルは親・子ともタグ（親は青灰 `#dce3ec`、子は既存タグ色。同じ親の子は直後のグループ）、「課金しています」直後は `.tags` 横並び。その他の入れ子は親＋子タグ。リスト文言は省略しない
...
1. 前回指摘のフラットリスト（遊び 3 件・業務の担当 2 件・今後挑戦したいこと）の 2 列と、業務リストと見出し 3 の重複は未着手。ユーザー指示があれば対応
